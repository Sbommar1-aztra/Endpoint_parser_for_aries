...
async def parse_swagger_file(
    ...
    try:
        ...
        # Normalize endpoint data to ensure all fields are strings, not None
        normalized_endpoints = []
        for ep in endpoints:
            normalized_ep = {
                'method': str(ep.get('method', '')) if ep.get('method') else '',
                'endpoint': str(ep.get('endpoint', '')) if ep.get('endpoint') else '',
                'summary': str(ep.get('summary', '')) if ep.get('summary') else '',
                'description': str(ep.get('description', '')) if ep.get('description') else '',
                'payload_schema': str(ep.get('payload_schema', '')) if ep.get('payload_schema') else '',
                'response_schema': str(ep.get('response_schema', '')) if ep.get('response_schema') else '',
                'tags': ep.get('tags', []) if isinstance(ep.get('tags'), list) else []
            }
            normalized_endpoints.append(normalized_ep)
        
        # Phase 0C: Analyze Dependencies
        try:
            dependency_analyzer = DependencyAnalyzer()
            dependencies = dependency_analyzer.analyze(normalized_endpoints)
        except Exception as e:
            # Log the error but continue with empty dependencies
            import traceback
        ...
        try:
            ...
            suggestions = suggestion_generator.generate_suggestions(normalized_endpoints)
        except Exception as e:
            # Log the error but continue with empty suggestions
            import traceback
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Unexpected error parsing file: {error_detail}. Please check the file format and try again."
        )
